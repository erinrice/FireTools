...
<System.Runtime.InteropServices.ProgID("CALM_FireTools.clsFire_Copy_2_ply")> _ 
<VB6Object("CALM_FireTools.clsFire_Copy_2_ply")> _
Public Class clsFire_Copy_2_ply
	Implements IDisposable
	Implements SystemUI.ICommand
' UPGRADE_INFO (#0501): The 'clsFire_Copy_2_ply' member isn't used anywhere in current application.
	...
	Private Const c_ModuleFileName As String = "CopyToPerimCmd"
	' UPGRADE_INFO (#0561): The 'TOOL_NAME' symbol was defined without an explicit "As" clause.
	Private Const TOOL_NAME As String = "Copy to Perimeter Layer"
	' UPGRADE_INFO (#0561): The 'TOOL_MSG' symbol was defined without an explicit "As" clause.
	Private Const TOOL_MSG As String = "Copy selected polygons from any layer into the Fire Boundary layer"
...
44:
	...
	Private ReadOnly Property ICommand_Bitmap() As Integer Implements SystemUI.ICommand.Bitmap
		Get
			On Error Resume Next
49:
			Return GetPictureHandle6(frmImages.copypoly.Picture)
	 	End Get
	End Property
...
54:
	...
	Private ReadOnly Property ICommand_Tooltip() As String Implements SystemUI.ICommand.Tooltip
		Get
			On Error Resume Next
59:
			Return "Copy Polygon(s) to Fire Boundary Layer"
	 	End Get
	End Property
...
77:
	...
	Private Sub ICommand_OnClick() Implements SystemUI.ICommand.OnClick
		...
		'ensure there is a selection to copy over
		If pMxDoc.FocusMap.SelectionCount = 0 Then
			MsgBox6("There are no features selected")
			Exit Sub
		End If
...
132:
		pFeatureLayer = modFire_Tools.Get_Fire_Poly_Layer()
...
153:
		...
		If pNewPerimFeatSet.Count = 0 Then
			...
			Exit Sub
		End If
		'  Declare some variables we'll need inside our do loop.
...
169:
		...
		End If
        Dim myFormPoly As New frmFire_Poly

        myFormPoly.txtRegion_Ex.Text = myIncidentInfo.IncidentRegionEx
        myFormPoly.txtFireNumber.Text = myIncidentInfo.IncidentNumber
        myFormPoly.txtRegion.Text = myIncidentInfo.incidentRegion
        myFormPoly.txtUserName.Text = getUserName()
		
		If modGlobals.g_UserDept = g_Fire_Dept.DEC Then
            myFormPoly.txtDept.Text = "DEC"
		ElseIf modGlobals.g_UserDept = g_Fire_Dept.FESA Then
            myFormPoly.txtDept.Text = "FESA"
		End If
		
        myFormPoly.txtDate.Text = mydatestring
        myFormPoly.txtTime.Text = mytimestring

        myFormPoly.ShowDialog()
        'myFormPoly.Show(VBRUN.FormShowConstants.vbModal)
        If myFormPoly.Did_Cancel = True Then
            mEditor.StopOperation(TOOL_NAME)
            Exit Sub
        End If
		
        Dim feature_string As String = myFormPoly.Feature_type_cb.Text
        ...
        pNewPerimFeat.Value(captmethodfield) = myFormPoly.cbCapt_Method.Text
        pNewPerimFeat.Value(capttimefield) = myFormPoly.txtTime.Text
        pNewPerimFeat.Value(captdatefield) = myFormPoly.txtDate.Text
        pNewPerimFeat.Value(userdeptfield) = myFormPoly.txtDept.Text
        pNewPerimFeat.Value(usernamefield) = myFormPoly.txtUserName.Text
        pNewPerimFeat.Value(regexfield) = myFormPoly.txtRegion_Ex.Text
        pNewPerimFeat.Value(regfield) = myFormPoly.txtRegion.Text
		pNewPerimFeat.Value(featfield) = feature_string
		pNewPerimFeat.Value(r_from) = m_datestring
        pNewPerimFeat.Value(firenumfield) = myFormPoly.txtFireNumber.Text
