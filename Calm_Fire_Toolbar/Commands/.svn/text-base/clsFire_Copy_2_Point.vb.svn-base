...
<System.Runtime.InteropServices.ProgID("CALM_FireTools.clsFire_Copy_2_Point")> _ 
<VB6Object("CALM_FireTools.clsFire_Copy_2_Point")> _
Public Class clsFire_Copy_2_Point
	Implements IDisposable
	Implements SystemUI.ICommand
' UPGRADE_INFO (#0501): The 'clsFire_Copy_2_Point' member isn't used anywhere in current application.
	...
	Private Const c_ModuleFileName As String = "clsCopy_2_Point"
	' UPGRADE_INFO (#0561): The 'TOOL_NAME' symbol was defined without an explicit "As" clause.
	Private Const TOOL_NAME As String = "Copy to Fire Point Layer"
	' UPGRADE_INFO (#0561): The 'TOOL_MSG' symbol was defined without an explicit "As" clause.
	Private Const TOOL_MSG As String = "Copy selected points from any layer into the Fire Point layer."
...
44:
	...
	Private ReadOnly Property ICommand_Bitmap() As Integer Implements SystemUI.ICommand.Bitmap
		Get
			On Error Resume Next
49:
			Return GetPictureHandle6(frmImages.copypoint.Picture)
	 	End Get
	End Property
...
54:
	...
	Private ReadOnly Property ICommand_Tooltip() As String Implements SystemUI.ICommand.Tooltip
		Get
			On Error Resume Next
59:
			Return "Copy Point(s) to Fire Point Layer"
	 	End Get
	End Property
...
132:
		pFeatureLayer = modFire_Tools.Get_Fire_Point_Layer()
...
150:
        ...
        If myForm.AppendToCurrent = True Then
            pQueryFilter.WhereClause = "[Eff_To] = 'Active'" 'changed query 19/09/06
            myfeatures = myfeatclass.Search(pQueryFilter, False)
...
169:
        ...
        Dim myFormPoint As New frmFire_Point
        myFormPoint.txtRegion_Ex.Text = myIncidentInfo.IncidentRegionEx
        myFormPoint.txtFireNumber.Text = myIncidentInfo.IncidentNumber
        myFormPoint.txtRegion.Text = myIncidentInfo.incidentRegion
        myFormPoint.txtUserName.Text = getUserName()
		
        myFormPoint.populatePointCB()
		
		If modGlobals.g_UserDept = g_Fire_Dept.DEC Then
            myFormPoint.txtDept.Text = "DEC"
		ElseIf modGlobals.g_UserDept = g_Fire_Dept.FESA Then
            myFormPoint.txtDept.Text = "FESA"
		End If
		
        myFormPoint.txtDate.Text = mydatestring
        myFormPoint.txtTime.Text = mytimestring
        myFormPoint.ShowDialog()
        'myFormPoint.Show(VBRUN.FormShowConstants.vbModal)
		
        If myFormPoint.Did_Cancel = True Then
            mEditor.StopOperation(TOOL_NAME)
            Exit Sub
        End If
		
        Dim feature_string As String = myFormPoint.Feature_type_cb.Text
        ...
        pNewPerimFeat.Value(captmethodfield) = myFormPoint.cbCapt_Method.Text
        pNewPerimFeat.Value(capttimefield) = myFormPoint.txtTime.Text
        pNewPerimFeat.Value(captdatefield) = myFormPoint.txtDate.Text
        pNewPerimFeat.Value(userdeptfield) = myFormPoint.txtDept.Text
        pNewPerimFeat.Value(usernamefield) = myFormPoint.txtUserName.Text
        pNewPerimFeat.Value(regexfield) = myFormPoint.txtRegion_Ex.Text
        pNewPerimFeat.Value(regfield) = myFormPoint.txtRegion.Text
		pNewPerimFeat.Value(featfield) = feature_string
		pNewPerimFeat.Value(r_from) = m_datestring
        pNewPerimFeat.Value(firenumfield) = myFormPoint.txtFireNumber.Text
