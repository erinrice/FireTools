...
<System.Runtime.InteropServices.ProgID("CALM_FireTools.clsFire_Capture_Div")> _ 
<VB6Object("CALM_FireTools.clsFire_Capture_Div")> _
Public Class clsFire_Capture_Div
	Implements SystemUI.ICommand
' UPGRADE_INFO (#0501): The 'clsFire_Capture_Div' member isn't used anywhere in current application.
	...
	Private ReadOnly Property ICommand_Enabled() As Boolean Implements SystemUI.ICommand.Enabled
		Get
            ...
            '			If modFire_Tools.Fire_Div_Exists() And modFire_Tools.Fire_Poly_Exists() Then
            '				bEnabled = True 'modFire_Tools.FCIsEditable(modFire_Tools.Get_Fire_Poly_Layer)
            '			Else
            '				bEnabled = False
            '			End If
            '			If g_Username = "" Then
...
ErrorHandler:
	...
	Private ReadOnly Property ICommand_Tooltip() As String Implements SystemUI.ICommand.Tooltip
		Get
			...
			' TODO: Add your implementation here
			Return "Create Fire Assignment Points for Fire Boundary"
...
ErrorHandler:
	...
	Private ReadOnly Property ICommand_Bitmap() As Integer Implements SystemUI.ICommand.Bitmap
		Get
			...
			' TODO: Add your implementation here
			Return GetPictureHandle6(frmImages.fire_div.Picture)
...
ErrorHandler:
	...
	Private Sub ICommand_OnClick() Implements SystemUI.ICommand.OnClick
        ...
        If g_incidentStatus = False Then
            ...
            Exit Sub
        End If
        If (modFire_Tools.Fire_Div_Exists() And modFire_Tools.Fire_Poly_Exists()) = False Then
            MsgBox("Either Fire Assignment or Boundary layers is missing")
            Exit Sub
        End If
        ...
        For count = 0 To pMap.LayerCount - 1
            If TypeOf pMap.Layer(count) Is ESRI.ArcGIS.Carto.IGroupLayer Then
                ...
                For complayercount = 0 To pcomplayer.Count - 1
                    ...
                    ElseIf pcomplayer.Layer(complayercount).Name = layername_fire_bound Then
                        pFeatLayer_fire_bound = pcomplayer.Layer(complayercount)
                    End If
                Next
            End If
        Next
        ...
        Dim mydataset As ESRI.ArcGIS.Geodatabase.IDataset = pFeatLayer.FeatureClass
        ...
        For taskcount = 0 To peditor.TaskCount - 1
            pEditTask = peditor.Task(taskcount)
            If pEditTask.Name = "CALM Fire Assignment" Then
        ...
        'snapping
        Dim pSnapEnvironment As ESRI.ArcGIS.Editor.ISnapEnvironment = peditor
        Dim pFeatSnap As ESRI.ArcGIS.Editor.IFeatureSnapAgent = New ESRI.ArcGIS.Editor.FeatureSnap()

        pSnapEnvironment.SnapToleranceUnits = ESRI.ArcGIS.Editor.esriSnapToleranceUnits.esriSnapTolerancePixels
        pSnapEnvironment.SnapTolerance = 20

        pSnapEnvironment.ClearSnapAgents() 'clear all existing snap agents
        'create a new feature snap agent
        pFeatSnap.FeatureClass = pfeatclass_fire_bound 'set to that of CurrentLayer
        pFeatSnap.HitType = ESRI.ArcGIS.Geometry.esriGeometryHitPartType.esriGeometryPartBoundary

        pSnapEnvironment.AddSnapAgent(pFeatSnap)
